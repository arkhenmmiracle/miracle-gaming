...
import {createOrder,paySimulation,HttpError} from './orders.js';
...
export function createApp(db,{origin=process.env.APP_ORIGIN||'http://localhost:3000',simulation=process.env.PAYMENT_MODE!=='production',production=process.env.NODE_ENV==='production'}={}){
 ...
 app.post('/api/orders/:id/simulate-payment',auth,async(req,res)=>{if(!simulation)throw new HttpError(404,'Tidak ditemukan.');res.json(await paySimulation(db,idSchema.parse(req.params.id),req.user.id));});
